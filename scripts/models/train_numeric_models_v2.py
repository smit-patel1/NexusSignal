"""
UPGRADED Phase 3: Advanced Numeric Model Training with SOTA Techniques

Improvements over v1:
- Advanced preprocessing (log returns, outlier clipping, rolling normalization)
- Data quality filtering (bad ticks, volume anomalies)
- Expanded alpha features (volatility, skew, kurtosis, RSI, MACD, ADX, beta, regimes)
- Rolling expanding window validation
- Horizon-specific feature engineering
- Model diversity (Ridge, Lasso, RandomForest, HistGradientBoosting + original models)
- Advanced neural architectures (DeepResidualLSTM, AdvancedTCN, Transformer)
- Cross-validated stacking with meta-features
- Target preprocessing (log transform, normalization, optional Kalman filtering)
...
TRAIN_NEURAL = True  # Set False to skip neural models
NEURAL_TICKERS = TICKERS[:10]  # Train neural only for subset
LOOKBACK_WINDOW = 32  # Sequence length

# Advanced preprocessing
USE_KALMAN_FILTER = False  # Kalman filtering for target denoising
TARGET_TRANSFORM = "log"  # 'log', 'arctanh', or 'winsorize'
NORMALIZE_TARGETS = True  # Normalize targets per horizon

# Data quality
FILTER_BAD_DATA = True  # Remove bad ticks and volume anomalies
MIN_VOLUME_PERCENTILE = 1.0  # Minimum volume percentile
MAX_PRICE_JUMP = 20.0  # Maximum price jump %

# Feature engineering
BUILD_ALPHA_FEATURES = True  # Add advanced alpha features
INCLUDE_INTERACTIONS = True  # Add interaction features

# Validation
USE_EXPANDING_WINDOW = True  # Use expanding window validation
N_CV_SPLITS = 5  # Number of CV splits
VALIDATION_GAP = 1  # Gap between train/test

# Ensemble
USE_STACKING = True  # Use cross-validated stacking
USE_META_FEATURES = True  # Augment with meta-features
STACKING_FOLDS = 5  # Folds for stacking

# Data paths
DATA_DIR = Path(__file__).parent.parent.parent / "data"
FEATURES_DIR = DATA_DIR / "processed" / "features"
MODELS_DIR = Path(__file__).parent.parent.parent / "models" / "numeric_v2"
...
def create_horizon_specific_features(
    ...
    Args:
        df: Input DataFrame
        horizon: Target horizon ('1h', '4h', '24h')
    ...
    # Add horizon-specific rolling features
    ...
    # Add momentum indicators scaled to horizon
    if "close" in df_horizon.columns:
...
def train_gradient_boosting_models(
    ...
    Returns:
        Dictionary with models and validation predictions
    """
    ...
    for model_name, model in base_models.items():
        ...
        try:
            ...
            if model_name == "lightgbm":
                ...
                # Linear and tree models without early stopping
...
def train_neural_model(
    ...
    Args:
        ...
        model_type: 'deep_lstm', 'advanced_tcn', or 'transformer'
    ...
    if model_type == "deep_lstm":
        ...
        print("    Training TransformerRegressor...")
...
def train_ticker_models_v2(
    ticker: str,
    market_returns: Optional[pd.Series] = None,
    train_neural: bool = TRAIN_NEURAL,
) -> Optional[Dict]:
    """
    UPGRADED training pipeline for a single ticker.

    Args:
        ticker: Stock ticker
        market_returns: Market returns for beta features
        train_neural: Whether to train neural models

    Returns:
        Trained artifacts or None if failed
    """
    print("\n" + "=" * 80)
    print(f"Training UPGRADED models for {ticker}")
    print("=" * 80)

    # Load and preprocess data
    df = load_and_preprocess_data(ticker, market_returns)
    if df is None:
        return None
    ...
    # Train separate pipeline for each horizon
    for target in TARGETS:
        ...
        feature_cols = [
            ...
            if c not in TARGETS and c != "ticker"
        ]
        X = df_horizon[feature_cols].copy()
        y = df_horizon[target].copy()
        ...
        if len(X) < 100:
            print(f"  [SKIP] Insufficient data for {target}")
            continue
        ...
        y_test_original = y.iloc[val_end:]  # For evaluation in original scale
        ...
        # Train neural models (if enabled and ticker in subset)
        if train_neural and ticker in NEURAL_TICKERS:
            ...
            # Train multiple neural architectures
        ...
        if USE_STACKING and len(horizon_artifacts["base_models"]) > 2:
            ...
            try:
                # Use only non-neural models for stacking (to avoid shape mismatch)
                stacking_models = {
                    k: v
    ...
    # Save artifacts
    artifact_path = MODELS_DIR / f"{ticker}_numeric_ensemble_v2.pkl"
...
def main() -> None:
    print("=" * 80)
    print("NEXUSSIGNAL PHASE 3: UPGRADED NUMERIC MODEL TRAINING")
    print("=" * 80)
    ...
    print(f"  Kalman filtering: {USE_KALMAN_FILTER}")
    ...
    for ticker in available_tickers:
        ...
        result = train_ticker_models_v2(
            ticker, market_returns=None, train_neural=train_neural_for_ticker
        )
    ...
    summary_path = MODELS_DIR / "training_summary_v2.csv"
